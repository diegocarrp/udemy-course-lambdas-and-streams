...
//        return stream.filter( videogame -> !videogame.getIsDiscount( ) )
//                .filter( videogame -> videogame.getPrice( ) < 9.99 )
//                .anyMatch( videogame -> videogame.getTotalSold( ) > 10 );
    ...
    static Stream<String> exercise4( Stream<Videogame> stream ) {
        return stream.map( Videogame::getReviews )
                .flatMap( reviews -> reviews.stream( ).map( Review::getComment ) );
    }
